...
if [ $# -eq 0 ];
then
	echo -e >&2 "\x1b[31m\x1b[1mPlease specify an argument. For all valid arguments, use the --help parameter.\x1b[0m"
	exit 1
else
	for i in "$@"
	do
		case "$i" in
			...
			*)
				...
				echo -e >&2 "\x1b[31m\x1b[1mThe script '$1' hasn't been implemented or you specified an invalid argument. Type --help to see all valid arguments.";
				exit 1;;
	esac
	done
fi